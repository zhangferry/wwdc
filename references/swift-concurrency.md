...
Swift 语言演进、async/await、Actor、Sendable 与线程安全。本领域覆盖 43 场 WWDC Session，回答时优先把 API 变化放回年度语境里判断。
...
`Swift` (4)、`开发工具` (3)、`基础` (2)、`Span` (1)、`InlineArray` (1)、`并发` (1)、`标准库` (1)、`系统服务` (1)、`SwiftUI` (1)
...
- [WWDC22-110351] 使用 Swift 并发消除数据竞争：Doug Gregor 用航海比喻系统性地讲解了 Swift 并发模型如何通过隔离（Isolation）和 Sendable 协议在编译期防止数据竞争——这是理解 Swift 6 严格并发检查的基础。
